...
func (m *Map) batchLookup(cmd sys.Cmd, startKey, nextKeyOut, keysOut, valuesOut interface{}, opts *BatchOptions) (int, error) {
	...
	ct, sysErr := sys.Raw(cmd, unsafe.Pointer(&attr), unsafe.Sizeof(attr))
...
func MapGetNextID(startID MapID) (MapID, error) {
	id, err := sys.BPF(&sys.MapGetNextIdAttr{Id: uint32(startID)})
	return MapID(id), err
}
...
func NewMapFromID(id MapID) (*Map, error) {
	fd, err := sys.BPFFd(&sys.MapGetFdByIdAttr{
		Id: uint32(id),
	})
	if err != nil {
		return nil, err
...
func mapBPF(attr sys.Attr) (int, error) {
	ret, err := sys.BPF(attr)
	return int(ret), wrapMapError(err)
}
